...
@section('content')
    ...
    <section class="bg-surface pb-20 md:pb-28">
        <div class="mx-auto max-w-6xl px-4 md:px-10 lg:px-12">
            ...
            <div class="mt-16 rounded-2xl bg-primary-dark p-8 text-center sm:p-10"><h2 class="font-serif text-xl font-bold text-white sm:text-2xl">Need Professional Advice?</h2><p class="mx-auto mt-2 max-w-lg text-sm text-white/70 sm:text-base">Our veterinary team is available for consultations. Book an appointment or call us directly.</p><div class="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row"><a href="{{ route('contact') }}?intent=veterinary&service=vet-care" class="w-cta w-cta--primary !bg-secondary !text-primary-dark hover:!bg-secondary-hover">Book an Appointment</a><a href="tel:{{ preg_replace('/\s+/', '', config('waggies.phone_international')) }}" class="w-cta w-cta--secondary border-white/30 bg-transparent text-white hover:bg-white/10">Call {{ config('waggies.phone') }}</a></div></div>
        </div>
    </section>
@endsection
