...
@section('content')
    ...
    <div x-data="faqPage(@js($faqs), @js($categories))" class="mx-auto max-w-5xl px-4 py-12 md:px-10 md:py-20">
        ...
        <div x-show="!query.trim()" x-transition class="mb-10 flex flex-wrap gap-2" role="tablist" aria-orientation="horizontal" aria-label="FAQ categories" @keydown="tabKeydown($event)">
            <button id="faq-tab-all" type="button" role="tab" aria-controls="faq-panel" @click="select('all')" :aria-selected="active === 'all'" :tabindex="active === 'all' ? 0 : -1" class="min-h-[44px] rounded-full px-5 py-2.5 text-sm font-semibold transition" :class="active === 'all' ? 'bg-primary text-white shadow-sm' : 'bg-surface-purple text-primary-dark hover:bg-primary/10'">All</button>
            <template x-for="category in categories" :key="category"><button :id="tabId(category)" type="button" role="tab" aria-controls="faq-panel" @click="select(category)" :aria-selected="active === category" :tabindex="active === category ? 0 : -1" class="min-h-[44px] rounded-full px-5 py-2.5 text-sm font-semibold capitalize transition" :class="active === category ? 'bg-primary text-white shadow-sm' : 'bg-surface-purple text-primary-dark hover:bg-primary/10'" x-text="category.replace('-', ' ')"></button></template>
        </div>
