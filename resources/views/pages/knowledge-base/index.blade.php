        ...
        <nav x-show="totalPages() > 1" role="navigation" aria-label="Pagination Navigation" class="mt-12 flex items-center justify-between gap-4 flex-wrap">
            ...
            <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between gap-4 flex-wrap">
                ...
                <div class="flex items-center gap-1">
                    ...
                    <template x-for="number in pageNumbers()" :key="number"><template x-if="number === page"><span class="inline-flex items-center justify-center w-9 h-9 text-sm font-semibold text-white bg-primary rounded-full shadow-sm" aria-current="page" x-text="number"></span></template><template x-if="number !== page"><a :href="pageHref(number)" :aria-label="`Go to page ${number}`" class="inline-flex items-center justify-center w-9 h-9 text-sm font-medium text-primary rounded-full hover:bg-primary/10 transition" x-text="number"></a></template></template>
