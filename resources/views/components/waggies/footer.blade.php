...
@php
    ...
    $footerColumns = [
        ...
        'Company' => [
            ['About', route('about')],
            ['Testimonials', route('about.testimonials')],
            ['Gallery', route('about.gallery')],
            ['Careers', route('about.careers')],
            ['Partnerships', route('about.partnerships')],
        ],
        'Resources' => [
            ['FAQ', route('faq')],
...
<footer class="mt-auto border-t border-white/10 bg-primary-dark">
        ...
        <div class="mx-auto max-w-7xl px-4 pb-8 pt-12 sm:px-6 sm:pt-16 lg:px-8 lg:pt-20">
            <div class="mb-12 rounded-2xl border border-white/10 bg-white/4 p-6 sm:p-8 lg:mb-14">
                <div class="grid items-center gap-6 lg:grid-cols-2 lg:gap-10">
                    <div>
                        <h3 class="font-serif text-2xl font-bold text-white text-balance sm:text-[1.75rem]">Pet care
                            tips, <span class="text-secondary italic">monthly.</span></h3>
                        <p class="mt-2 max-w-md text-sm text-white/65 sm:text-base">Pet care tips and updates from
                            Waggies. No spam, unsubscribe anytime.</p>
                    </div>
                    <form action="{{ route('newsletter.store') }}" method="post"
                        class="flex flex-col gap-3 sm:flex-row" x-data="{ submitting: false }" @submit="submitting = true">
                        @csrf
                        <div class="hidden" aria-hidden="true">
                            <label for="newsletter-website">Leave this field empty</label>
                            <input id="newsletter-website" name="website" type="text" tabindex="-1"
                                autocomplete="off">
                        </div>
                        <div class="relative flex-1">
                            <label class="sr-only" for="footer-email">Email address</label>
                            <x-waggies.icon name="email" size="20"
                        ...
                        <x-waggies.button type="submit"
                            ...
                            <span x-show="!submitting" class="inline-flex items-center gap-2"><x-waggies.icon
                                    name="send" size="16" />Subscribe</span>
                            <span x-show="submitting" x-cloak
                                class="inline-flex items-center gap-2">Sending&hellip;</span>
                        </x-waggies.button>
                    </form>
                    @if ($errors->has('email'))
                        <p class="mt-3 text-sm font-medium text-red-200" role="alert">{{ $errors->first('email') }}
                        </p>
                    @endif
                        ...
                        <ul class="space-y-3">
                            @foreach ($links as [$label, $href])
                                <li><a href="{{ $href }}"
                                        ...
                                        @if (str_starts_with($href, 'http')) target="_blank" rel="noopener noreferrer" @endif>{{ $label }}</a>
                                </li>
                            @endforeach
                        </ul>
                        @if ($heading === 'Support')
    ...
    <div class="border-t border-white/10">
        <div class="mx-auto flex max-w-7xl flex-col items-center justify-between gap-3 px-4 py-5 sm:flex-row sm:px-6">
            <p class="text-xs text-white/60 sm:text-sm/6">© 2026 Waggies Pet Services. All rights reserved.</p>
            <nav aria-label="Legal" class="flex items-center gap-4 sm:gap-5"><a href="{{ route('privacy-policy') }}"
                    class="text-xs text-white/60 hover:text-white sm:text-sm/6">Privacy</a><a
                    href="{{ route('terms-of-service') }}"
                    class="text-xs text-white/60 hover:text-white sm:text-sm/6">Terms</a><a
                    href="{{ route('cookies-policy') }}"
                    class="text-xs text-white/60 hover:text-white sm:text-sm/6">Cookies</a></nav>
        </div>
    </div>
</footer>
