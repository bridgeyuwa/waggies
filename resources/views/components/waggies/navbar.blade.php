...
<header x-data="waggiesNavbar" class="sticky top-0 z-layer-navigation w-full overflow-visible border-b border-primary/10 bg-white transition-[background-color,box-shadow,border-color] duration-200" @click.outside="closeDesktop()" @keydown.escape.window="escape()">
    <nav x-ref="nav" class="page-container flex min-h-18 items-center justify-between overflow-visible" aria-label="Main navigation">
        ...
        <ul class="hidden items-center gap-1 overflow-visible lg:flex" role="list">
            @foreach ($navigation as $key => $menu)
                ...
                <li class="relative overflow-visible" @mouseenter="hoverOpen('{{ $key }}')" @mouseleave="hoverClose('{{ $key }}')">
                    <button type="button" id="nav-trigger-{{ $key }}" data-nav-trigger="{{ $key }}" class="flex min-h-[44px] items-center gap-1 rounded-full px-4 py-2 text-[0.8125rem] font-medium uppercase tracking-[0.08em] transition-colors {{ $current ? 'bg-surface-purple text-primary font-semibold' : 'text-primary-dark/60 hover:bg-surface-purple hover:text-primary' }}" aria-haspopup="true" aria-expanded="false" aria-controls="desktop-menu-{{ $key }}" :aria-expanded="desktopMenu === '{{ $key }}'" @click.stop="toggle('{{ $key }}')" @keydown.arrow-down.prevent="openAndFocus('{{ $key }}')">
                        {{ $menu['label'] }}
                        <x-waggies.icon name="chevron-down" size="14" class="transition-transform duration-200" x-bind:class="desktopMenu === '{{ $key }}' && 'rotate-180'" />
                    </button>

                    @if (($menu['kind'] ?? 'stacked') === 'mega')
                        <div id="desktop-menu-{{ $key }}" x-cloak x-show="desktopMenu === '{{ $key }}'" x-effect="if (desktopMenu === '{{ $key }}') updateMegaOffset('{{ $key }}')" x-transition:enter="transition-[opacity,translate,scale,rotate] duration-200 ease-out motion-reduce:transition-none" x-transition:enter-start="opacity-0 -translate-y-1 scale-95 motion-reduce:translate-y-0 motion-reduce:scale-100" x-transition:enter-end="opacity-100 translate-y-0 scale-100" x-transition:leave="transition-[opacity,translate,scale,rotate] duration-100 ease-in motion-reduce:transition-none" x-transition:leave-start="opacity-100 translate-y-0 scale-100" x-transition:leave-end="opacity-0 -translate-y-1 scale-95 motion-reduce:translate-y-0 motion-reduce:scale-100" class="nav-dropdown nav-dropdown--mega origin-top absolute top-full mt-1 will-change-transform" role="region" aria-labelledby="nav-trigger-{{ $key }}" :aria-hidden="desktopMenu !== '{{ $key }}'" @mouseenter="cancelClose()" @mouseleave="hoverClose('{{ $key }}')">
                            ...
                            <div class="nav-dropdown__hub">
                                <a href="{{ $destination($menu) }}" class="nav-dropdown__hub-link group" @click="closeDesktop()" @if($isExact($menu)) aria-current="page" @endif>
                                    ...
                                    <div role="group" aria-label="{{ $column['label'] }}">
                                        ...
                                        <div class="flex flex-col gap-0.5">
                                            @foreach ($column['items'] as $item)
                                                ...
                                                <a href="{{ $destination($item) }}" class="nav-mega-link group {{ ($item['variant'] ?? '') === 'overview' ? 'nav-mega-link--overview' : '' }} {{ ($item['variant'] ?? '') === 'child' ? 'nav-mega-link--child' : '' }} {{ $itemContainsCurrent ? 'nav-mega-link--contains-current' : '' }}" @click="closeDesktop()" @if($itemCurrent) aria-current="page" @endif>
                    ...
                    @else
                        <div id="desktop-menu-{{ $key }}" x-cloak x-show="desktopMenu === '{{ $key }}'" x-transition:enter="transition-[opacity,translate,scale,rotate] duration-200 ease-out motion-reduce:transition-none" x-transition:enter-start="opacity-0 -translate-y-1 scale-95 motion-reduce:translate-y-0 motion-reduce:scale-100" x-transition:enter-end="opacity-100 translate-y-0 scale-100" x-transition:leave="transition-[opacity,translate,scale,rotate] duration-100 ease-in motion-reduce:transition-none" x-transition:leave-start="opacity-100 translate-y-0 scale-100" x-transition:leave-end="opacity-0 -translate-y-1 scale-95 motion-reduce:translate-y-0 motion-reduce:scale-100" class="nav-dropdown nav-dropdown--stacked origin-top absolute right-0 top-full mt-1 will-change-transform" role="region" aria-labelledby="nav-trigger-{{ $key }}" :aria-hidden="desktopMenu !== '{{ $key }}'" @mouseenter="cancelClose()" @mouseleave="hoverClose('{{ $key }}')">
                            ...
                            <div class="nav-dropdown__stacked-links">
                                @foreach ($menu['items'] as $item)
                                    <a href="{{ $destination($item) }}" class="nav-mega-link group" @click="closeDesktop()" @if($isExact($item)) aria-current="page" @endif>
                                        <span class="nav-mega-link__icon"><x-waggies.icon name="{{ $item['icon'] }}" size="18" /></span>
                                        <span class="min-w-0"><span class="nav-mega-link__title">{{ $item['label'] }}</span><span class="nav-mega-link__subtitle">{{ $item['description'] }}</span></span>
                                    </a>
                                @endforeach
        ...
        <div class="flex items-center gap-3">
            ...
            <button x-ref="mobileToggle" x-show="!mobileOpen" type="button" class="flex h-11 w-11 flex-col items-center justify-center gap-[5px] rounded-lg p-3 transition-colors hover:bg-surface-purple lg:hidden" aria-expanded="false" :aria-expanded="mobileOpen" aria-label="Open navigation menu" aria-controls="mobile-menu" @click="openMobile()">
                <span class="block h-0.5 w-5 rounded-full bg-primary-dark" aria-hidden="true"></span>
                <span class="block h-0.5 w-5 rounded-full bg-primary-dark" aria-hidden="true"></span>
                <span class="block h-0.5 w-5 rounded-full bg-primary-dark" aria-hidden="true"></span>
            </button>
        </div>
    ...
    <div x-cloak x-show="mobileOpen" x-transition:enter="transition-opacity duration-300 ease-out motion-reduce:transition-none" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition-opacity duration-200 ease-in motion-reduce:transition-none" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 z-layer-sticky bg-primary-dark/55 lg:hidden" aria-hidden="true" @click="closeMobile()"></div>
    <aside id="mobile-menu" x-cloak x-show="mobileOpen" x-transition:enter="transition-[opacity,translate,scale,rotate] duration-300 ease-out motion-reduce:transition-none" x-transition:enter-start="translate-x-full opacity-0 motion-reduce:translate-x-0" x-transition:enter-end="translate-x-0 opacity-100" x-transition:leave="transition-[opacity,translate,scale,rotate] duration-200 ease-in motion-reduce:transition-none" x-transition:leave-start="translate-x-0 opacity-100" x-transition:leave-end="translate-x-full opacity-0 motion-reduce:translate-x-0" x-ref="drawer" class="mobile-nav-drawer fixed inset-y-0 right-0 z-layer-navigation flex max-w-sm flex-col overflow-hidden rounded-l-2xl border-l border-nav-border bg-white shadow-modal lg:hidden" role="navigation" aria-label="Main navigation" tabindex="-1" :aria-hidden="!mobileOpen" @click.stop>
        <div class="sticky top-0 z-10 flex items-center justify-between border-b border-nav-border bg-white/95 px-6 py-4 backdrop-blur-sm">
        ...
        <nav class="flex flex-1 flex-col gap-1.5 overflow-y-auto px-3 py-5" aria-label="Mobile navigation links">
            ...
            <div x-data="{ open: false }">
                ...
                <div id="mobile-services-submenu" x-cloak x-show="open" x-transition:enter="transition-[opacity,translate,scale,rotate] duration-200 ease-out motion-reduce:transition-none" x-transition:enter-start="opacity-0 -translate-y-1 motion-reduce:translate-y-0" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition-[opacity,translate,scale,rotate] duration-100 ease-in motion-reduce:transition-none" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-1 motion-reduce:translate-y-0" class="overflow-hidden pl-2" :aria-hidden="!open">
                        ...
                        @foreach ($navigation['services']['columns'] as $column)
                            @if (in_array($column['label'], ['Boarding', 'Relocation'], true))
                                ...
                                <div x-data="{ expanded: false }" role="group" aria-label="{{ $column['label'] }}">
                                    ...
                                    <div id="mobile-{{ strtolower($column['label']) }}-submenu" x-cloak x-show="expanded" x-transition:enter="transition-[opacity,translate,scale,rotate] duration-200 ease-out motion-reduce:transition-none" x-transition:enter-start="opacity-0 -translate-y-1 motion-reduce:translate-y-0" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition-[opacity,translate,scale,rotate] duration-100 ease-in motion-reduce:transition-none" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-1 motion-reduce:translate-y-0" class="ml-3 overflow-hidden border-l-2 border-surface-purple pl-4" :aria-hidden="!expanded">
            ...
            @foreach (['about', 'tools', 'resources'] as $key)
                ...
                <div x-data="{ open: false }">
                    ...
                    <div id="mobile-{{ $key }}-submenu" x-cloak x-show="open" x-transition:enter="transition-[opacity,translate,scale,rotate] duration-200 ease-out motion-reduce:transition-none" x-transition:enter-start="opacity-0 -translate-y-1 motion-reduce:translate-y-0" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition-[opacity,translate,scale,rotate] duration-100 ease-in motion-reduce:transition-none" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-1 motion-reduce:translate-y-0" class="overflow-hidden pl-2" :aria-hidden="!open">
                        <div class="flex flex-col gap-0.5 py-1">
                            @if ($key === 'tools')
                                ...
                                @foreach ($menu['columns'] as $column)
                                    <div x-data="{ expanded: false }">
                                        ...
                                        <div id="mobile-tools-{{ strtolower(str_replace(' ', '-', $column['label'])) }}" x-cloak x-show="expanded" x-transition:enter="transition-[opacity,translate,scale,rotate] duration-200 ease-out motion-reduce:transition-none" x-transition:enter-start="opacity-0 -translate-y-1 motion-reduce:translate-y-0" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition-[opacity,translate,scale,rotate] duration-100 ease-in motion-reduce:transition-none" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-1 motion-reduce:translate-y-0" class="ml-3 overflow-hidden border-l-2 border-surface-purple pl-4" :aria-hidden="!expanded">@foreach ($column['items'] as $item)<a href="{{ $destination($item) }}" class="mobile-nav-sublink text-primary-dark/65" @click="closeMobile()">{{ $item['label'] }}</a>@endforeach</div>
                                    </div>
                                @endforeach
                            @else
