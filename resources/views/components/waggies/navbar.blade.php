...
<header x-data="waggiesNavbar" class="sticky top-0 z-50 w-full overflow-visible border-b border-primary/10 bg-white transition-[background-color,box-shadow,border-color] duration-200" @click.outside="closeDesktop()" @keydown.escape.window="escape()">
    ...
    <aside id="mobile-menu" x-cloak x-show="mobileOpen" x-transition x-ref="drawer" class="fixed right-0 top-0 z-50 flex h-full w-80 max-w-[90vw] flex-col bg-white shadow-2xl lg:hidden" role="navigation" aria-label="Main navigation" tabindex="-1" :aria-hidden="!mobileOpen" @click.stop>
        ...
        <nav class="flex flex-1 flex-col gap-1 overflow-y-auto px-4 py-4" aria-label="Mobile navigation links">
            <button type="button" class="mb-2 flex min-h-[44px] w-full items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold text-primary-dark transition-colors hover:bg-surface-purple" aria-label="Search the site" @click="$dispatch('waggies:open-search'); closeMobile()"><x-waggies.icon name="search" size="16" class="text-primary-dark/70" /><span class="flex-1 text-left">Search the site</span><kbd class="hidden items-center rounded-md border border-surface-purple bg-white px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide text-primary-dark/60 sm:inline-flex" aria-hidden="true">⌘K</kbd></button>
