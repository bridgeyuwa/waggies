...
final class KnowledgeArticle extends Model implements HasMedia, HasRichContent
{
    use HasSlug;
    use HasUuids;
    ...
    protected static function booted(): void
    {
        self::saving(function (self $article): void {
            if (! in_array($article->status, array_keys(self::statusOptions()), true)) {
                throw new InvalidArgumentException("Invalid Knowledge Article status [{$article->status}].");
            }
        ...
        self::creating(function (self $article): void {
            $article->assertSlugIsNotOwnedByAnotherArticle();
        });

        self::updating(function (self $article): void {
            $article->assertSlugIsNotOwnedByAnotherArticle();
        });

        self::updated(function (self $article): void {
            if ($article->wasChanged('slug')) {
                KnowledgeArticleSlugHistory::firstOrCreate([
    ...
    /**
     * Limit Knowledge Articles to records that may render through the normal public route.
     *
    ...
    /**
     * Limit Knowledge Articles to records that may participate in public search.
     *
     * @param  Builder<self>  $query
     * @return Builder<self>
     */
    ...
    /**
     * Limit Knowledge Articles to records eligible for the public sitemap.
     *
     * @param  Builder<self>  $query
     * @return Builder<self>
     */
    ...
    /**
     * Adapt the persisted record to the existing fixed Blade Knowledge Base contract.
     *
    ...
    private function assertSlugIsNotOwnedByAnotherArticle(): void
    {
        if (blank($this->slug)) {
            return;
        }

        $historyQuery = KnowledgeArticleSlugHistory::query()->where('slug', $this->slug);

        if ($this->exists) {
            $historyQuery->where('knowledge_article_id', '!=', $this->getKey());
        }

        if ($historyQuery->exists()) {
            throw new InvalidArgumentException("The Knowledge Article slug [{$this->slug}] is reserved by a previous Knowledge Article URL.");
        }
    }
}
